...
>
> 本文件曾记录具体路径与账户名；入库版本已换成占位符，本机的真值另存于
> `local-data/LOCAL-PATHS.md`（该目录已 gitignore，不进仓库）。
...
| `<用户目录>\.gradle` | 幸存 | 含 760 MB 依赖缓存 + Gradle 9.1.0 发行版 |
| `<用户目录>\.android` | 幸存 | adbkey 等 |
| `<Android SDK 路径>` | 幸存 | 完整 SDK（在非系统盘，未受影响） |
| `<JDK 21 路径>` | 幸存 | Temurin 21.0.9 LTS，本次选作 JAVA_HOME |
| 用户环境变量 / PATH | **重置** | 这是"装了却找不到"的根因 |
| `<用户目录>\.gitconfig` | 不存在 | 无全局 git 身份，需仓库级配置 |

## 3. 工具清单（含验证状态）

| 工具 | 状态 | 路径 | 版本 | 实测 |
| --- | --- | --- | --- | --- |
| JDK（**已选定**） | 可用 | `<JDK 21 路径>` | Temurin 21.0.9+10-LTS | 通过 `java` / `javac -version` |
| JDK（备选） | 可用 | `<JDK 17 路径>` | Temurin 17.0.2+8 | 仅版本探测 |
| JDK（备选） | 可用 | `<JDK 22 路径>` | Oracle 22 | 版本偏新，未验证 |
| Android SDK | 可用 | `<Android SDK 路径>` | — | `sdk.dir` 已被既有工程使用 |
...
| CMake（独立） | 可用 | `<独立 CMake 路径>\bin\cmake.exe` | 4.3.2 | 未实测 |
...
本机另一个 Flutter 工程的 Android 壳（`<本机另一个工程>`）成功构建过，其配置可作为 T0.2 的已知可用组合参照：
...
| `ANDROID_NDK_ROOT` | `<Android SDK 路径>\ndk\28.2.13676358` |
| `HOME` | `<用户目录>`（修复 adb 的 `Cannot mkdir '\.android'`） |
| `PATH`（追加） | `<JDK 21 路径>\bin`、`<Android SDK 路径>\platform-tools`、`<Android SDK 路径>\cmdline-tools\latest\bin` |

原有用户 PATH 条目全部保留。

**风险提示**：选定的 JDK 21 来自第三方软件自带的目录，路径不带版本号所以相对稳定，但它仍属于第三方软件目录。若将来卸载或大版本升级那个软件，`JAVA_HOME` 会失效。建议后续单独安装一份独立的 Temurin LTS 到固定目录（如 `C:\Java\jdk-21`）再改指向。
...
本会话的沙箱运行在**独立的 Windows 账户** `<主机名>\CodexSandboxOffline` 下，与你的账户 `<主机名>\<用户>` 不同。这带来两个必须提权才能完成的动作：
...
注意：沙箱内创建的文件属主是 `CodexSandboxOffline`，提权执行的命令以你的账户运行。这曾导致 git 的 `dubious ownership` 报错，已通过"以你的账户身份重建仓库"解决。
